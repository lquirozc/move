...
import { CustomErrorsService } from '../services/custom-errors.service';
...
export class HttpConfigInterceptor implements HttpInterceptor {
    ...
    constructor(public errorDialogService: DialogService, public loaderService: LoaderService,
        ...
        private errorsService: CustomErrorsService) { }
        ...
        return next.handle(request).pipe(
            ...
            catchError((error: HttpErrorResponse) => {
                ...
                if (data["status"] != "" && data["status"] == "401") {
                    ...
                    //Internal server
                    this.errorsService.getErrorsDictionary('500').then(resError => {
                        this.errorDialogService.openDialog('error', resError);
                    }, err => {
                        this.errorDialogService.openDialog('warning', 'Error app');
                    });
                    return throwError('error-->>>' + error);
                     ...
                     if (error.error.error_description != undefined || error.error.message != undefined) {
                        ...
                        const errCode = error.error.error_description ? error.error.error_description : error.error.message;

                        this.errorsService.getErrorsDictionary(errCode).then(resError => {
                            this.errorDialogService.openDialog('error', resError);
                        }, err => {
                            this.errorDialogService.openDialog('error', errCode);
                        });
                    }
                    ...
                    //Not found
                    this.errorsService.getErrorsDictionary('404').then(resError => {
                        this.errorDialogService.openDialog('error', resError);
                    }, err => {
                        this.errorDialogService.openDialog('warning', 'Error app');
                    });
                    ...
                    //Conflict
                    this.errorsService.getErrorsDictionary(error.error).then(resError => {
                        this.errorDialogService.openDialog('error', resError);
                    }, err => {
                        this.errorDialogService.openDialog('warning', 'Error app');
                    });
                    ...
                    this.errorsService.getErrorsDictionary('002').then(resError => {
                        this.errorDialogService.openDialog('error', resError);
                    }, err => {
                        this.errorDialogService.openDialog('warning', 'Internet error');
                    });
